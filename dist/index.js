import { stepField } from "@workspace/eros-eris-field";
function sleep(ms) {
            ...
            if (now - lastEmbed >= embedEveryMs) {
                ...
                if (candidate) {
                    ...
                    try {
                        ...
                        // eslint-disable-next-line no-console
                        console.log(`[eros-eris] embedded ${candidate.id} · vec=${vec.length} · semanticPairs=${semanticPairs.size} · nodes=${particlesById.size}`);
                    }
                    catch (err) {
